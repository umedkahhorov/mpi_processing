...
def mpiHeaders(filename=None,segy_header=None,out=None):
    ...
    if segy_header=='CDP_X':
        ssegy_header=segyio.TraceField.CDP_X
    if segy_header=='CDP_Y':
        ssegy_header=segyio.TraceField.CDP_Y
    if segy_header=='INLINE_3D':
        ssegy_header=segyio.TraceField.INLINE_3D
    if segy_header=='CROSSLINE_3D':
        ssegy_header=segyio.TraceField.CROSSLINE_3D
    if segy_header=='SourceX':
        ssegy_header=segyio.TraceField.SourceX
    if segy_header=='SourceY':
        ssegy_header=segyio.TraceField.SourceY
    if segy_header=='GroupX':
        ssegy_header=segyio.TraceField.GroupX
    if segy_header=='GroupY':
        ssegy_header=segyio.TraceField.GroupY
    ...
    header = f.attributes(ssegy_header)[start_idx:end_idx]
    ...
    if rank == 0:
        np.savetxt(out, recvbuf, delimiter=',', header=segy_header, comments='')
    # Close the SEGY file
    f.close()
    t2 = time.time()
    total_time = t2 - t1
